...
function forkedTest(name, leftOpts, rightOpts) {
  ...
  try {
    leftIds = adapter.ids(left)
  } catch (e) {
    console.log(`CRASH left: ${e.message}`)
    return
  }
  try {
    rightIds = adapter.ids(right)
  } catch (e) {
    console.log(`CRASH right: ${e.message}`)
    return
  }
  const converged = idsEqual(leftIds, rightIds)
  console.log(`${converged ? 'PASS' : 'FAIL'}: ${name}`)
  if (!converged)
    console.log(`  sizes: left=${leftIds.length} right=${rightIds.length}`)
}

forkedTest(
  'latency forked (middle vs tail, mixed)',
  { mixed: true, position: 'middle' },
  { mixed: true, position: 'tail' }
)
forkedTest(
  'collaborative offline (middle vs tail, mixed)',
  { mixed: true, position: 'middle' },
  { mixed: true, position: 'tail' }
)
forkedTest(
...
forkedTest(
  'insert only, middle vs tail',
  { mixed: false, position: 'middle' },
  { mixed: false, position: 'tail' }
)
forkedTest(
  'insert only, both middle',
  { mixed: false, position: 'middle' },
  { mixed: false, position: 'middle' }
)
forkedTest(
  'insert only, tail vs middle',
  { mixed: false, position: 'tail' },
  { mixed: false, position: 'middle' }
)
