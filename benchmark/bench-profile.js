...
function value(id) { return { id, payload: { text: `value:${id}`, number: id } } }

// Setup: create list, take snapshot, create remote
function setup() {
  const source = __create()
  for (let i = 0; i < LIST_SIZE; i++) __update(source.size, [value(i)], source, 'after')
  return source
}
...
// Profile: pure sequential scan
{
  ...
  console.log(`Sequential scan: ${ms.toFixed(2)}ms total, ${(ms / SCAN_OPS).toFixed(3)}ms/op`)
  console.log(`  ~${Math.round(totalReads / SCAN_OPS)} reads/op, ${(ms * 1e6 / totalReads).toFixed(1)}ns/read`)
}
...
// Profile: just merge without scan
{
  ...
  const ms = Number(process.hrtime.bigint() - start) / 1e6
  console.log(`Merge only: ${ms.toFixed(2)}ms total, ${(ms / SCAN_OPS).toFixed(3)}ms/op`)
}
...
for (let i = 0; i < remote3.size; i++) __read(i, remote3)  // warm cache
{
  ...
  console.log(`Cache hit reads: ${ms.toFixed(2)}ms for ${reads} reads = ${(ms * 1e6 / reads).toFixed(1)}ns/read`)
}
...
const remote4 = adapter.hydrate(snapshot)
{
  ...
  console.log(`Cache miss reads: ${ms.toFixed(2)}ms for ${reads} reads = ${(ms * 1e6 / reads).toFixed(1)}ns/read`)
}
