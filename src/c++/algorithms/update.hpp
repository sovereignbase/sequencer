 ...
 * uses index zero and an insert type. Mask spans follow the addressed source.
 */
inline std::uint32_t update_projection(
  ...
  const auto positioned_strip = operation_type == 2
                                    ? containing_strip_index
                                    : incoming_strip_index;
  const auto position = find_projection_frame_index_of(
      projector, positioned_strip, frame_count_diff, strip_count_diff);
  projector.gate_strip_index = positioned_strip;
  ...
  projection_buffer.write_projection(
      0, {projector.strip_type_of[incoming_strip_index],
          ...
          u32_max, u32_max});
  return operation_type == 2 ? operation_index : position;
}
