 ...
 *
 * LengthTable groups stable Sequence Point keys by Realm and stores each
 * Realm's Strips in counter order. Realm lookup uses open addressing with
 * linear probing; exact point lookup within one Realm uses binary search. This
 * keeps the common Realm-local insertion and traversal paths contiguous in
 * memory.
 */
#pragma once
 ...
 * @note LengthTable stores coordinates exactly as supplied. Projector
 * algorithms derive runtime structural links without modifying them.
 */
...
public:
   ...
   * operation or destruction of this LengthTable.
   ...
   * remains outside LengthTable; this operation applies the supplied result.
   *
   ...
   * @post This LengthTable and all structural Projector state remain unchanged.
