...
std::uint32_t find_strip_by_sequence_point(ProjectorState *projector,
                                           const SeqeuncePoint *strip_strart,
  ...
  while (offset == max_uint32) {
    ...
    std::int8_t comparison_result =
        compare_sequence_point(this_strip_start, strip_strart);
    ...
    this_strip_start =
        &projector->reel[projector->gate_strip_start_position].this_strip_start;
    // Set offset
    offset = current_strip_contains_sequence_point(strip_start, strip_start,
                                                   strip_length);
  }
  return offset;
}
