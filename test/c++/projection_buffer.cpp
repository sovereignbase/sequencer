...
int main() {
  ...
  for (std::uint32_t strip_index = 0; strip_index < 3; ++strip_index) {
    ...
    for (std::uint32_t word_index = 0; word_index < 10; ++word_index)
      words[word_index] = 0x80000000u + strip_index * 10 + word_index;
  ...
  for (std::uint32_t strip_index = 0; strip_index < 3; ++strip_index)
    for (std::uint32_t word_index = 0; word_index < 10; ++word_index)
      assert(received[strip_index][word_index] ==
             0x80000000u + strip_index * 10 + word_index);
  assert(buffer.get_word_count() == 50);
  buffer.clear();
  assert(buffer.get_strip_count() == 0);
  assert(buffer.get_memory_pointer() == nullptr);
}
