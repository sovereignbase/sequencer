...
describe('transfer validation', () => {
  it('accepts only the current eight-word Delta shape', () => {
    const insertion = [[7], [1, 0, 1, 0, 0, 0, 7, 2], ['a']]
    const mask = [[7], [2, 0, 1, 0, 7, 2, 8, 2]]
    expect(is_delta(insertion)).toBe(true)
    expect(is_delta(mask)).toBe(true)
    expect(is_delta(null)).toBe(false)
    expect(is_delta([[7], [1, 0, 1, 0, 0, 0, 7, 2]])).toBe(false)
    expect(is_delta([[7], [2, 0, 1, 0, 7, 2, 8, 2], []])).toBe(false)
    expect(is_delta([[7], [1, 0, 1, 0, 0, 0, -1, 2], ['a']])).toBe(false)
  })
