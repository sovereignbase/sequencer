...
void clip_strip_at_offset(Projector *projector,
  ...
  if (offset > 0) {
    ...
    Strip clipped_strip = projector->reel[strip_to_clip_position];

    clipped_strip.length =
        projector->reel[strip_to_clip_position].length - offset;
    clipped_strip.timecode.add(offset);
    clipped_strip.footage_code += offset;
